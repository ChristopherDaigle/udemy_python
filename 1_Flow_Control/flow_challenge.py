...
def flow_chal():
    """
    Function to print the number and length of segments in an ip address
    ...
    if ip_address[-1] != '.':
        segment_length = 0
        for _ in ip_address[len(lengths) + sum(lengths):]:
            segment_length += 1
    lengths.append(segment_length)

    print("The IP Address {0} has {1} segments of {2} lengths each."
          .format(ip_address, segments, lengths)
          )
